...
class TeamTracking():
    def __init__(self, tracking_file_path=None,
        ...
        if tracking_file_path:
            ...
            raise ValueError(
                "L'utilisateur doit fournir au minimum un lien vers un fichier de données .jsonl ou un dataframe de données déstructurées de SecondSpectrum.")
    ...
    def calculate_acceleration(self, smoothing=False, window=7, polyorder=1, maxacceleration=12):
        ...
        if False:
            ...
            # Vitesse scalaire
            self.df_tracking.loc[:, 'vx'] = self.df_tracking.groupby(['optaId', 'period']).vx.apply(
                lambda x: signal.savgol_filter(x, window_length=window, polyorder=polyorder))
            self.df_tracking.loc[:, 'vy'] = self.df_tracking.groupby(['optaId', 'period']).vy.apply(
                lambda x: signal.savgol_filter(x, window_length=window, polyorder=polyorder))
        ...
        self.df_tracking.loc[:, 'acceleration'] = self.df_tracking.groupby(
            ['optaId', 'period']).speed.diff() / self.frequence
        self.df_tracking.loc[self.df_tracking.acceleration > maxacceleration, 'acceleration'] = np.nan
        # Smoothing
        if smoothing:
            self.df_tracking.loc[:, 'acceleration'] = self.df_tracking.groupby(
                ['optaId', 'period']).acceleration.transform(
                lambda x: signal.savgol_filter(x, window_length=window, polyorder=polyorder))
    ...
    def calculate_metabolic_cost(self, smoothing=False, window=7, polyorder=1):
        ...
        self.df_tracking.loc[mask_positive_acc, 'metabolic_cost'] = self.df_tracking.loc[
                                                                        mask_positive_acc, 'metabolic_cost'] * (
                                                                                4.03 * self.df_tracking.loc[
                                                                            mask_positive_acc, 'acceleration'] + 3.6 * np.exp(
                                                                            -0.408 * self.df_tracking.loc[
                                                                                mask_positive_acc, 'acceleration']))

        # Calcul pour les accélérations négatives
        mask_negative_acc = self.df_tracking.acceleration < 0
        self.df_tracking.loc[mask_negative_acc, 'metabolic_cost'] = self.df_tracking.loc[
                                                                        mask_negative_acc, 'metabolic_cost'] * (
                                                                                -0.85 * self.df_tracking.loc[
                                                                            mask_negative_acc, 'acceleration'] + 3.6 * np.exp(
                                                                            1.33 * self.df_tracking.loc[
                                                                                mask_negative_acc, 'acceleration']))

        # Smoothing
        if smoothing:
            self.df_tracking.loc[:, 'metabolic_cost'] = self.df_tracking.groupby(
                ['optaId', 'period']).metabolic_cost.transform(
                lambda x: signal.savgol_filter(x, window_length=window, polyorder=polyorder))
